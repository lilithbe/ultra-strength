...
const AppConfig = (props) => {
    ...
    // const [theme, setTheme] = useState(process.env.REACT_APP_DEFAULT_TEHME);
    ...
    useEffect(() => {
        ...
        const themeHref = 'https://lilithbe.github.io/post-admin/assets/themes/' + props.theme + '/theme.css';
        replaceLink(themeElement, themeHref);

    },[replaceLink, props.theme])
    ...
    return (
        <div ref={config} className={configClassName} id={"layout-config"}>
            ...
            <div className="layout-config-content">
                ...
                <h5>Themes</h5>
                    {themeList.map((themes,ti)=>{
                        return(
                            <div key={ti} >
                                    <h6 className="mt-0">{themes.label}</h6>
                                <div className="grid free-themes">
                            
                                    {themes.items.map((theme,i)=>{
                                        return(
                                            <div key={i} className="col-3 text-center">
                                            <button className="p-link" onClick={e => changeTheme(e, theme.theme, theme.colorMode)}>
